...
import {
  Edit2, Trash2, ExternalLink, Loader2,
  Eye, Tag, Search, Filter, Plus, Newspaper,
} from "lucide-react";
...
export default function NoticiasAdminPage() {
  ...
  async function handleDelete(id: string) {
    if (!window.confirm("Apagar essa matéria permanentemente?")) return;
    const { error } = await supabase.from("articles").delete().eq("id", id);
    if (!error) setNews(prev => prev.filter(n => n.id !== id));
    else alert("Erro ao remover matéria.");
  }
  ...
  return (
    <div className="flex flex-col gap-16 max-w-7xl mx-auto pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 border-b-2 border-slate-900 pb-12">
        ...
        <Link
          href="/admin/publicar"
          className="bg-slate-950 text-white px-10 py-5 text-[11px] uppercase tracking-[0.3em] font-black hover:bg-accent transition-all duration-300 shadow-[6px_6px_0px_0px_rgba(249,115,22,1)] flex items-center gap-3"
        >
            ...
            <AnimatePresence mode="popLayout">
              {filteredNews.map((item, index) => (
                ...
                >
                  ...
                  {/* Thumbnail */}
                  <div className="w-full md:w-56 h-40 bg-slate-50 border border-slate-100 shrink-0 overflow-hidden relative grayscale group-hover:grayscale-0 transition-all duration-700 shadow-sm group-hover:shadow-[8px_8px_0px_0px_rgba(249,115,22,1)] group-hover:-translate-y-1">
                    {item.image_url ? (
                      <Image src={item.image_url} alt="" fill className="object-cover transition-transform duration-1000 group-hover:scale-110" />
                    ) : (
                  ...
                  <div className="flex-1 flex flex-col gap-4">
                    {/*
                      URL relativa → o browser resolve para o domínio atual automaticamente.
                      target="_blank" é tratado pelo browser nativamente, não pelo roteador Next.js.
                    ...
                    */}
                    ...
                    <div className="flex items-center gap-4 mt-auto pt-6 border-t border-slate-50 flex-wrap">
                      ...
                      <div className="flex items-center gap-2">
                        {/* Botão VER — sempre renderizado, URL relativa nativa */}
                        <a
                          href={`/noticia/${item.slug}/`}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex items-center gap-2 px-5 py-3 border-2 border-accent text-accent text-[9px] font-black uppercase tracking-widest hover:bg-accent hover:text-white transition-all"
                        >
